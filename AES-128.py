...
def input_data():
    ...
    # type_input_data = int(input("Вводим с клавиатуры или из файла? (1/2): "))
    # if type_input_data == 1:
    string = input("Введите текст: ")
    # else:
    #     path_to_file = input(
    #         "Введите путь (D:\Учёба\8 семестр (-)\(-) Информационная безопасность\Лабораторные работы\Лабораторная работа №3 _\Лабораторная работа №3\lab_3_1.txt): ")
    #     with open(path_to_file, "r") as file:      # encoding='UTF-8'
    #         string = file.read()
    ...
    if choices_function == 1:
        ...
        while True:
            if result[-1] == 40:
                ...
                break
        print('Дешифрованные байты {0}: '.format(result))
# Точка входа
main()
